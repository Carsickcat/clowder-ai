...
async function bundleApplication() {
  const sources = await Promise.all(
    applicationFiles.map((relativePath) =>
      readFile(join(rootDirectory, "src", relativePath), "utf8"),
    ),
  );
...
async function readStyles() {
  const sources = await Promise.all(
    styleFiles.map((fileName) =>
      readFile(join(rootDirectory, "src", fileName), "utf8"),
    ),
  );
  return sources.join("\n");
}
...
export async function buildStandalone(options = {}) {
  const outputFile =
    typeof options === "string"
      ? options
      : options.outputPath ?? join(rootDirectory, artifactName);
