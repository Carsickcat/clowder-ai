...
### 使用 Tailscale Serve 的所有者认证

远程派任务与审批应让 API 只监听本机回环地址，由 Tailscale Serve 认证浏览器身份：
...
`FRONTEND_URL` 必须填写 Tailscale Serve 显示的精确 HTTPS origin，不能带 path、query 或 fragment。可信 Tailscale 身份头不能与 `API_SERVER_HOST=0.0.0.0` 同时启用；API 会拒绝启动，避免网络客户端绕过 Serve 后伪造身份头。

在 Tailnet 中启用 HTTPS Certificates 后，用同一 origin 代理前端、API 与 Socket.IO（端口替换为 `.env` 中的 `FRONTEND_PORT` 和 `API_SERVER_PORT`）：
...
代理 target 末尾的 `/api` 和 `/socket.io` 不能省略：`--set-path` 转发时会移除公开挂载前缀，因此 target URL 必须把前缀补回去。
