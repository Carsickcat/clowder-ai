...
const TEST_SESSION_TOKEN = 'a'.repeat(64);
const TEST_PRINCIPAL = {
  dataUserId: 'default-user',
  authSubject: 'local:default-user',
  authMethod: 'local',
  roles: ['owner'],
};
 ...
 */
function attemptConnection(port, { origin, transports = ['websocket'], auth, sessionToken = TEST_SESSION_TOKEN } = {}) {
  return new Promise((resolve) => {
    const extraHeaders = {
      ...(origin ? { origin } : {}),
      ...(sessionToken ? { cookie: `cat_cafe_session=${sessionToken}` } : {}),
    };
...
describe('F156: WebSocket Origin Security (integration)', () => {
  ...
  before(async () => {
    ...
    socketManager = new SocketManager(httpServer, undefined, undefined, {
      verifySessionToken: (token) => (token === TEST_SESSION_TOKEN ? TEST_PRINCIPAL : null),
    });
    socketManager.setThreadAccessResolver(async (threadId, dataUserId) => {
      return threadId === 'owned-thread' && dataUserId === 'default-user';
    });
  ...
  it('rejects an unconfigured Tailscale Serve HTTPS origin by default', async () => {
    const result = await attemptConnection(port, {
      origin: 'https://ragdoll.example.ts.net',
      transports: ['websocket'],
    });
    assert.strictEqual(result.connected, false, 'Tailscale Serve origin must require exact FRONTEND_URL opt-in');
  });
  ...
  it('rejects a connection without a valid session cookie', async () => {
    ...
    assert.strictEqual(result.connected, false, 'anonymous socket must be rejected');
  });

  it('allows an owned thread room and denies a foreign thread room', async () => {
    const result = await attemptConnection(port, {
      origin: 'http://localhost:3003',
    });
    assert.strictEqual(result.connected, true);

    result.socket.emit('join_room', 'thread:owned-thread');
    result.socket.emit('join_room', 'thread:foreign-thread');
    await new Promise((r) => setTimeout(r, 50));

    const sockets = await socketManager.getIO().fetchSockets();
    const target = sockets.find((s) => s.id === result.socket.id);
    assert.ok(target?.rooms.has('thread:owned-thread'));
    assert.ok(!target?.rooms.has('thread:foreign-thread'));
    result.socket.disconnect();
  });
...
describe('F156: configured Tailscale Serve origin (integration)', () => {
  it('allows only the exact FRONTEND_URL origin', async () => {
    const previousFrontendUrl = process.env.FRONTEND_URL;
    process.env.FRONTEND_URL = 'https://ragdoll.example.ts.net';
    const server = createServer();
    const manager = new SocketManager(server, undefined, undefined, {
      verifySessionToken: (token) => (token === TEST_SESSION_TOKEN ? TEST_PRINCIPAL : null),
    });

    try {
      const configuredPort = await new Promise((resolve) => {
        server.listen(0, '127.0.0.1', () => resolve(server.address().port));
      });
      const allowed = await attemptConnection(configuredPort, {
        origin: 'https://ragdoll.example.ts.net',
      });
      assert.strictEqual(allowed.connected, true, 'exact FRONTEND_URL origin should connect');
      allowed.socket?.disconnect();

      const lookalike = await attemptConnection(configuredPort, {
        origin: 'https://evil.ragdoll.example.ts.net',
      });
      assert.strictEqual(lookalike.connected, false, 'lookalike origin must remain rejected');
    } finally {
      manager.close();
