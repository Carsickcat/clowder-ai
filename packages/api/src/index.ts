...
import { assertTrustedTailscaleServeConfig, sessionAuthPlugin, sessionRoute } from './infrastructure/session-auth.js';
...
async function main(): Promise<void> {
  assertTrustedTailscaleServeConfig(process.env);
  ...
  socketManager.setThreadAccessResolver(async (threadId, dataUserId) => {
    ...
    if (!thread) return threadId === 'default' && dataUserId === 'default-user';
    return thread.createdBy === dataUserId || thread.createdBy === 'system';
  });
