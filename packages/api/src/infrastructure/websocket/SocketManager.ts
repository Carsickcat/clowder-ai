...
export interface SocketManagerSecurityOptions {
  verifySessionToken?: (token: string) => SessionPrincipal | null;
}

export type ThreadAccessResolver = (threadId: string, dataUserId: string) => boolean | Promise<boolean>;
...
export class SocketManager {
  ...
  private readonly verifySessionToken: (token: string) => SessionPrincipal | null;
  private threadAccessResolver: ThreadAccessResolver | null;
  /**
  ...
  constructor(
    ...
    securityOptions?: SocketManagerSecurityOptions,
  ) {
    this.invocationTracker = invocationTracker ?? null;
    this.queueProcessor = null;
    this.multiMentionOrchestrator = null;
    this.verifySessionToken = securityOptions?.verifySessionToken ?? validateSessionToken;
    this.threadAccessResolver = null;
    ...
    // Origin validation and authentication are separate boundaries. Reuse the
    // HttpOnly HTTP session so Socket.IO cannot invent or self-report identity.
    this.io.use((socket, next) => {
      const cookieHeader = Array.isArray(socket.handshake.headers.cookie)
        ? socket.handshake.headers.cookie[0]
        : socket.handshake.headers.cookie;
      const token = readSessionTokenFromCookieHeader(cookieHeader);
      const principal = token ? this.verifySessionToken(token) : null;
      if (!principal) {
        log.warn({ socketId: socket.id }, 'Socket connection rejected: authenticated session required');
        next(new Error('Authenticated session required'));
        return;
      }
      socket.data.sessionPrincipal = principal;
      next();
    });
  ...
  private setupEventHandlers(): void {
    this.io.on('connection', (socket: Socket) => {
      ...
      const principal = socket.data.sessionPrincipal as SessionPrincipal;
      const userId = principal.dataUserId;
      const isOwner = principal.roles.includes('owner');
      log.info(
        { socketId: socket.id, userId, authSubject: principal.authSubject, authMethod: principal.authMethod },
        'Client connected',
      );
      log.debug(
        {
      ...
      socket.on('join_room', async (room: string) => {
        ...
        if (room.startsWith('thread:')) {
          ...
          if (!threadId || !this.threadAccessResolver) {
            log.warn({ socketId: socket.id, room, userId }, 'Thread room denied: access resolver unavailable');
            return;
          }
          try {
            if (!(await this.threadAccessResolver(threadId, userId))) {
              log.warn({ socketId: socket.id, room, userId }, 'Thread room ACL denied');
              return;
            }
          } catch (error) {
            log.warn({ socketId: socket.id, room, userId, error: String(error) }, 'Thread room ACL check failed');
            return;
          }
        }
        if ((room.startsWith('worktree:') || room === 'workspace:global' || room === 'preview:global') && !isOwner) {
          log.warn({ socketId: socket.id, room, authSubject: principal.authSubject }, 'Owner room ACL denied');
          return;
        }
        socket.join(room);
      ...
      socket.on('cancel_invocation', (data: { threadId: string; catId?: string }) => {
        if (!this.invocationTracker || !data?.threadId || !isOwner) return;
  ...
  /** Wire persisted thread ownership after the thread store is initialized. */
  setThreadAccessResolver(resolver: ThreadAccessResolver): void {
    this.threadAccessResolver = resolver;
  }
