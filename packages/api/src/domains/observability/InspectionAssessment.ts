...
export function projectInspectionABReport(runs: readonly InspectionRun[]): InspectionABReport | null {
  const baselineRun = runs.find((run) => run.purpose === 'admission');
  const currentRun = [...runs].reverse().find((run) => run.purpose === 'post_change');
  if (!baselineRun || !currentRun) return null;

  const checkIds = [
    ...new Set([...baselineRun.checkResults, ...currentRun.checkResults].map((result) => result.checkId)),
  ];
  ...
  const comparability =
    validRuns && comparableCount === checks.length && checks.length > 0
      ? 'valid'
      : validRuns && comparableCount > 0
        ? 'partial'
        : 'unavailable';
  return {
...
function recommendationFor(
  ...
  if (coverageStatus === 'omission') {
    return {
      ...
      evidenceRefs: candidateSet?.coverageOmissions.flatMap((omission) => omission.evidenceRefs) ?? [],
    };
  }
  return {
...
export function projectInspectionAssessment(
  ...
  const coverageStatus = (candidateSet?.coverageOmissions.length ?? 0) > 0 ? 'omission' : 'complete';
  ...
  return {
    ...
    unknowns: unknownsForRun(run, candidateSet, abReport),
    recommendations: [recommendationFor(run, candidateSet, coverageStatus, abReport)],
  };
}
