...
let latestTaskProgressSnapshotStartedAt = 0;

/** Epoch-based, strictly monotonic request version shared by every task-progress consumer. */
export function nextTaskProgressSnapshotStartedAt(): number {
  const now = typeof performance === 'undefined' ? Date.now() : performance.timeOrigin + performance.now();
  latestTaskProgressSnapshotStartedAt = Math.max(now, latestTaskProgressSnapshotStartedAt + 0.001);
  return latestTaskProgressSnapshotStartedAt;
}
...
function hasNewerConcurrentTaskProgress(
  info: CatInvocationInfo,
  incoming: TaskProgressState | undefined,
  snapshotStartedAt: number,
): boolean {
  const localUpdatedAt = info.taskProgress?.lastUpdate;
  if (localUpdatedAt === undefined || localUpdatedAt < Math.floor(snapshotStartedAt)) return false;
  return incoming === undefined || localUpdatedAt > incoming.lastUpdate;
}

function reconcileTaskProgressSnapshot(
  source: Record<string, CatInvocationInfo>,
  snapshot: Record<string, TaskProgressState>,
  snapshotStartedAt: number,
): Record<string, CatInvocationInfo> {
  const next: Record<string, CatInvocationInfo> = {};

  for (const [catId, info] of Object.entries(source)) {
    const incoming = snapshot[catId];
    if (hasNewerConcurrentTaskProgress(info, incoming, snapshotStartedAt)) {
      next[catId] = info;
    } else {
      next[catId] = { ...info, taskProgress: incoming };
    }
  }

  for (const [catId, taskProgress] of Object.entries(snapshot)) {
    if (!(catId in source)) next[catId] = { taskProgress };
  }
  return next;
}
...
export interface ChatState {
  ...
  /** Latest successfully applied task-progress request start, including authoritative empty snapshots. */
  _taskProgressSnapshotStartedAtByThread: Record<string, number>;
  ...
  /** Replace only task-progress snapshots for one thread, preserving all other invocation metadata. */
  reconcileThreadTaskProgress: (
    threadId: string,
    snapshot: Record<string, TaskProgressState>,
    snapshotStartedAt: number,
  ) => boolean;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  reconcileThreadTaskProgress: (threadId, snapshot, snapshotStartedAt) => {
    let applied = false;
    set((state) => {
      const previousStartedAt = state._taskProgressSnapshotStartedAtByThread[threadId];
      if (previousStartedAt !== undefined && snapshotStartedAt < previousStartedAt) return state;
      applied = true;
      const _taskProgressSnapshotStartedAtByThread = {
        ...state._taskProgressSnapshotStartedAtByThread,
        [threadId]: snapshotStartedAt,
      };

      if (threadId === state.currentThreadId) {
        const catInvocations = reconcileTaskProgressSnapshot(state.catInvocations, snapshot, snapshotStartedAt);
        return {
          catInvocations,
          _taskProgressSnapshotStartedAtByThread,
          ...mirrorActiveFlat(state, { catInvocations }),
        };
      }
      ...
      return {
        ...
        threadStates: {
          ...
          [threadId]: {
            ...existing,
            catInvocations: reconcileTaskProgressSnapshot(existing.catInvocations, snapshot, snapshotStartedAt),
          },
        },
      };
    });
