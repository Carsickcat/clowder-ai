...
describe('useChatHistory task-progress hydration', () => {
  ...
  it('does not resurrect task progress when an older history response arrives after a newer empty reconcile', async () => {
    let resolveTaskProgress!: (response: Response) => void;
    let markTaskProgressStarted!: () => void;
    const taskProgressStarted = new Promise<void>((resolve) => {
      markTaskProgressStarted = resolve;
    });
    const delayedTaskProgress = new Promise<Response>((resolve) => {
      resolveTaskProgress = resolve;
    });
    ...
    useChatStore.setState({
      catInvocations: {
        codex: {
          ...
          taskProgress: {
            tasks: [{ id: 'old', subject: 'Old work', status: 'in_progress' }],
    ...
    apiFetchMock.mockImplementation((url: string) => {
      ...
      if (typeof url === 'string' && url.includes('/task-progress')) {
        markTaskProgressStarted();
        return delayedTaskProgress;
      }
      if (typeof url === 'string' && url.includes('/queue')) {
        return Promise.resolve(new Response(JSON.stringify({ queue: [], paused: false }), { status: 200 }));
      }
    ...
    await act(async () => {
      await taskProgressStarted;
    });

    act(() => {
      useChatStore.getState().reconcileThreadTaskProgress('thread-progress', {}, Number.MAX_SAFE_INTEGER - 1);
    });
    expect(useChatStore.getState().catInvocations.codex?.taskProgress).toBeUndefined();
    ...
    await act(async () => {
      ...
      await delayedTaskProgress;
      await Promise.resolve();
      await Promise.resolve();
    });

    expect(useChatStore.getState().catInvocations.codex?.invocationId).toBe('old-invocation');
    expect(useChatStore.getState().catInvocations.codex?.taskProgress).toBeUndefined();
  });
});
