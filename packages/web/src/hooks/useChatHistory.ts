...
import type { QueueEntry, TaskProgressItem, TaskProgressState } from '@/stores/chat-types';
import {
  type CatInvocationInfo,
  type ChatMessage as ChatMessageData,
  nextTaskProgressSnapshotStartedAt,
  useChatStore,
...
export function useChatHistory(threadId: string) {
  ...
  const fetchTaskProgress = useCallback(async () => {
    ...
    try {
      ...
      const applied = useChatStore.getState().reconcileThreadTaskProgress(fetchForThread, snapshot, snapshotStartedAt);
      if (!applied) return;
  ...
  }, [threadId, replaceThreadTargetCats]);
