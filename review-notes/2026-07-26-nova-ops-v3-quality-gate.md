...
- 首次 Sites 构建成功后托管端按约定收取 `dist/`，而 Next 静态导出默认为 `out/`；已增加受测的 `out → dist` 构建产物同步。

## 最新验证证据

- `npm run check`：14/14 tests pass，Next production build / static export / Sites distribution exit 0。
