...
### Tailscale Serve with Owner Authentication

For remote task dispatch and approvals, keep the API private to the local machine and let Tailscale Serve authenticate the browser:

```bash
API_SERVER_HOST=127.0.0.1
FRONTEND_URL=https://your-machine.your-tailnet.ts.net
CAT_CAFE_TRUST_TAILSCALE_HEADERS=true
CAT_CAFE_REMOTE_OWNER_LOGIN=you@example.com
```

`FRONTEND_URL` must be the exact HTTPS origin shown by Tailscale Serve, without a path, query, or fragment. Trusted Tailscale identity headers and `API_SERVER_HOST=0.0.0.0` are intentionally incompatible: the API fails startup so a direct network client cannot bypass Serve and forge those headers.

After enabling HTTPS certificates for the tailnet, proxy the frontend, API, and Socket.IO on one origin (replace the ports with `FRONTEND_PORT` and `API_SERVER_PORT` from `.env`):

```bash
tailscale serve --yes --bg --https=443 http://127.0.0.1:<frontend-port>
tailscale serve --yes --bg --https=443 --set-path=/api http://127.0.0.1:<api-port>/api
tailscale serve --yes --bg --https=443 --set-path=/socket.io http://127.0.0.1:<api-port>/socket.io
tailscale serve status
```

The `/api` and `/socket.io` suffixes on the proxy targets are required: `--set-path` removes the public mount prefix before forwarding, so the target URL must add it back.
